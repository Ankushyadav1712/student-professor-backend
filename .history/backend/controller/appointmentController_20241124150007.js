...
const StudentAppointments = ({ studentId }) => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const response = await fetch(`/api/students/${studentId}/appointments`);
        const data = await response.json();

        if (data.appointments && data.appointments.length > 0) {
          setAppointments(data.appointments);
        } else {
          setMessage(data.message || "No appointments found.");
        }
      } catch (err) {
        ...
        <ul>
          {appointments.map((appointment) => (
            <li key={appointment._id}>
              {appointment.date} - {appointment.timeSlot} (Status: {appointment.status})
            </li>
          ))}
        </ul>
      )}
